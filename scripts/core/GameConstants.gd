...
const WAVE_MAX_CONCURRENT := 2           # حداکثر «گروه» هم‌زمان در صحنه (هر گروه = ناوگان چندقایقی)
const BOAT_SPEED := 2.2                  # سرعت پهلوگیری نهایی (m/s)
const BOAT_CRUISE_SPEED := 4.6           # سرعت کروز در دریای باز — ظهور از افق (گام ۶R2)
const BOAT_SPAWN_DIST := 32.0            # فاصله‌ی ظهور قایق از لنگر — نزدیک خط افق آب (m)
const BOAT_DOCK_ZONE := 8.0              # فاصله‌ی ترمز به سرعت پهلوگیری (m)
...
# ---------- گام ۶R2 — ناوگان مهاجم: چند نوع قایق (بازخورد کاربر) ----------
## «هر دسته با قایق‌های مختلف خودش می‌آید؛ بعضی کوچک‌تر و بدون بادبان، بعضی بزرگ‌تر»
const FLEET_BOAT_GAP := 2.2               # فاصله‌ی پهلوگیری قایق‌های یک ناوگان کنار هم (m)
const CAP_ROWBOAT := 3                    # قایق کوچک پارویی — بدون بادبان
const CAP_GALLEY := 6                     # گالی متوسط با بادبان
const CAP_WARSHIP := 8                    # کشتی جنگی بزرگ با بادبان بزرگ
