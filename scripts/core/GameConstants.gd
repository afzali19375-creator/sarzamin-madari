...
const BLOCK_SITE_SPACING := 2.5           # گام ۶R۱۳ — شبکه‌ی سایت‌ها متراکم‌تر (بازخورد: «پدها فاصله‌های کم داشته باشند») — قبلاً ۳٫۲
...
## گام ۶R۱۳ — «کاربر باید بتواند خودِ دوربین را جابه‌جا کند»: پنِ افقیِ محدود
const CAM_PAN_SIDE_MAX := 1.6             # متر — بیشترین لغزش چپ/راست
const CAM_PAN_SPEED := 4.0                # m/s — نرمی پن (کلیدهای جهت‌نما)
const CAM_PAN_DRAG_SENS := 0.012          # متر بر پیکسل — کشیدنِ موس/لمس
const CAM_NEAR := 0.1
const CAM_FAR := 200.0
